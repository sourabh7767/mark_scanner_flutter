...
class ApiClient {
  ...
  Future<bool> login(String email,String password) async {
    try{
      ...
      Response response=await _dio.post(ApiPath.loginPath,data: FormData.fromMap({
        "email":email,
        "password":password
      }));
...
return true;
 ...
 Future<bool> passwordChange(String oldPasseord,String newPassword,String confirmPassword) async {
    try{
      String auth =await LocalStorage.getString(LocalStorage.auth) ?? "";

      Response response=await _dio.post(ApiPath.changePasswordPath,data: FormData.fromMap({
...
return true;
  ...
  Future<bool> logOut() async {
    try{
String auth =await LocalStorage.getString(LocalStorage.auth) ?? "";
      Response response=await _dio.get(ApiPath.logoutPath,options: Options(
        headers: {
          "Authorization":"Bearer "+auth
        }
      ));
